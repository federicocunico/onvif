...
logging.basicConfig(filename='teste-onvif.log',
...
class CameraControl:
    ...
    def __init__(self, ip, user, password, wsdl_dir=None, port = None):
        self.__cam_ip = ip
        self.__cam_user = user
        self.__cam_password = password
        self.__port = int(port) if port is not None else 80
        
        if wsdl_dir is None:
            pwd = os.getcwd()
            wsdl_dir = os.path.join(pwd, "onvif_data", "onvif_specs", "specs", "wsdl")  # double nesting required for bug in onvif_zeep
            assert os.path.isdir(wsdl_dir), f"Unable to find the folder for WSDL ({wsdl_dir}), please provide it as argument"

        self.__wsdl_dir = wsdl_dir
    ...
    def camera_start(self):
        ...
        """
        args = [self.__cam_ip, self.__port, self.__cam_user, self.__cam_password]
        if self.__wsdl_dir is not None:
            args.append(self.__wsdl_dir)
        mycam = ONVIFCamera(*args)  # Some cameras use port 8080
